...
type TriggerResult struct {
	...
	NextExecutionDate NullableInt64 `json:"nextExecutionDate,omitempty"`
	// Time that will be waited between the next and the next+1 execution of this trigger
	NextDelay NullableString `json:"nextDelay,omitempty" validate:"regexp=^([0-9]+) *(ms|millisecond|milliseconds|s|second|seconds|m|minute|minutes|h|hour|hours|d|day|days)$"`
	// Contains details on this trigger's execution
	Detail NullableString `json:"detail,omitempty"`
	// Is this trigger manually retryable (can be [run](#tag/api.certificate/operation/certificate.run))
...
func (o *TriggerResult) GetNextDelay() string {
	if o == nil || IsNil(o.NextDelay.Get()) {
		var ret string
		return ret
	}
