...
func (dst *SystemConfigurationUpsertRequest) UnmarshalJSON(data []byte) error {
	...
	err = newStrictDecoder(data).Decode(&dst.InterfaceCustomizationConfiguration)
	if err == nil {
	...
	err = newStrictDecoder(data).Decode(&dst.InternalMonitorConfiguration)
	if err == nil {
	...
	err = newStrictDecoder(data).Decode(&dst.LicenseConfiguration)
	if err == nil {
