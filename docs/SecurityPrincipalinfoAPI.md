...
[**SecurityPrincipalInfoGet**](SecurityPrincipalinfoAPI.md#SecurityPrincipalInfoGet) | **Get** /api/v1/security/principalinfos/{identifier} | Retrieve a principal information
...
## SecurityPrincipalInfoGet

> PrincipalInfoResponse SecurityPrincipalInfoGet(ctx, identifier).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.SecurityPrincipalinfoAPI.SecurityPrincipalInfoGet(context.Background(), identifier).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SecurityPrincipalinfoAPI.SecurityPrincipalInfoGet``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `SecurityPrincipalInfoGet`: PrincipalInfoResponse
	fmt.Fprintf(os.Stdout, "Response from `SecurityPrincipalinfoAPI.SecurityPrincipalInfoGet`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiSecurityPrincipalInfoGetRequest struct via the builder pattern
