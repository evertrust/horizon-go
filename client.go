...
// APIClient manages communication with the Horizon API API v2.11.0
...
type APIClient struct {
	...
	AcmeAccountAPI *AcmeAccountAPIService

	AcmeEabAPI *AcmeEabAPIService

	AcmeEabpolicyAPI *AcmeEabpolicyAPIService

	AcmeOrderAPI *AcmeOrderAPIService
	...
	DcvLifecycleAPI *DcvLifecycleAPIService

	DcvPolicyAPI *DcvPolicyAPIService

	DcvProviderAPI *DcvProviderAPIService

	DcvProvisionerAPI *DcvProvisionerAPIService
	...
	ReportCsvAPI *ReportCsvAPIService
	...
	SecurityServiceAccountAPI *SecurityServiceAccountAPIService

	SecurityTeamAPI *SecurityTeamAPIService

	SecurityTenantAPI *SecurityTenantAPIService

	SystemConfigurationAPI *SystemConfigurationAPIService

	SystemStorageAPI *SystemStorageAPIService

	SystemTosAPI *SystemTosAPIService
...
func NewAPIClient(cfg *Configuration) *APIClient {
	...
	// API Services
	c.AcmeAccountAPI = (*AcmeAccountAPIService)(&c.common)
	c.AcmeEabAPI = (*AcmeEabAPIService)(&c.common)
	c.AcmeEabpolicyAPI = (*AcmeEabpolicyAPIService)(&c.common)
	c.AcmeOrderAPI = (*AcmeOrderAPIService)(&c.common)
	...
	c.DcvLifecycleAPI = (*DcvLifecycleAPIService)(&c.common)
	c.DcvPolicyAPI = (*DcvPolicyAPIService)(&c.common)
	c.DcvProviderAPI = (*DcvProviderAPIService)(&c.common)
	c.DcvProvisionerAPI = (*DcvProvisionerAPIService)(&c.common)
	...
	c.ReportCsvAPI = (*ReportCsvAPIService)(&c.common)
	...
	c.SecurityServiceAccountAPI = (*SecurityServiceAccountAPIService)(&c.common)
	c.SecurityTeamAPI = (*SecurityTeamAPIService)(&c.common)
	c.SecurityTenantAPI = (*SecurityTenantAPIService)(&c.common)
	c.SystemConfigurationAPI = (*SystemConfigurationAPIService)(&c.common)
	c.SystemStorageAPI = (*SystemStorageAPIService)(&c.common)
	c.SystemTosAPI = (*SystemTosAPIService)(&c.common)
...
func (c *APIClient) prepareRequest(
	...
	if ctx != nil {
		if saCreds, ok := ctx.Value(ContextServiceAccount).(*ServiceAccountCreds); ok && saCreds != nil {
			token, err := saCreds.tokenFn()
			if err != nil {
				return nil, fmt.Errorf("could not resolve service account token: %s", err.Error())
			}
			localVarRequest.Header.Set("X-API-SVA", saCreds.name)
			localVarRequest.Header.Set("X-API-TOKEN", token)
		}
	}
...
func (e *GenericOpenAPIError) As(target any) bool {
	if target == nil {
		return false
	}

	// If the caller is asking for GenericOpenAPIError itself
	if t, ok := target.(**GenericOpenAPIError); ok {
		*t = e
		return true
	}

	// If the model can populate the target
	if e.model != nil {
		...
		mv := reflect.ValueOf(e.model)
		if mv.Type().AssignableTo(tv.Elem().Type()) {
			tv.Elem().Set(mv)
			return true
		}
	}
...
func formatErrorMessage(status string, v interface{}) string {
	...
	if metaValue.Kind() == reflect.Struct {
		...
		if field != (reflect.Value{}) {
			// try the get method first, if it exists
